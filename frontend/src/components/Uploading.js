...
[
  ...
  useEffect(() => {
    // map over the array and add each object to the "books" collection
    menu.forEach(async (book) => {
      await addDoc(collection(db, `users/VMp7JD7fIkNPIX8pjnXX9zNKivH2/books`), book);
    });
  }, []);
  return (
    <div><>
        <button  className="pl-24 bg-white text-black">add all books</button>
    </></div>
  )
}
